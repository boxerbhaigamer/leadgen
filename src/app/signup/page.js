...
export default function SignupPage() {
    ...
    return (
        ...
        >
            <button onClick={toggleTheme} className="theme-toggle" style={{ position: "fixed", top: 20, right: 20, zIndex: 50 }} aria-label="Toggle theme">
            ...
            <div style={{ width: "100%", maxWidth: 420 }}>
                ...
                <div className="glass-card" style={{ padding: "36px 32px" }}>
                    <h1 style={{ fontSize: "1.5rem", fontWeight: 800, textAlign: "center", marginBottom: 4 }}>Create Your Account</h1>
                    <p style={{ color: "var(--color-text-muted)", textAlign: "center", fontSize: "0.9rem", marginBottom: 28 }}>Start scraping leads for free</p>
                    ...
                    {success && (
                        <div style={{ background: "rgba(0,206,201,0.1)", border: "1px solid rgba(0,206,201,0.3)", borderRadius: 10, padding: "10px 14px", marginBottom: 16, fontSize: "0.85rem", color: "#00CEC9" }}>
                            {success}
                        </div>
                    )}

                    {/* Google */}
                    <button onClick={handleGoogleSignup} disabled={loading} style={{ ...googleBtnStyle, opacity: loading ? 0.6 : 1 }}>
                    ...
                    {/* Email */}
                    {method === "email" && (
                        <form onSubmit={handleEmailSignup} style={{ display: "grid", gap: 14 }}>
                            <div>
                            ...
                            <div>
                                ...
                                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min 6 characters" minLength={6} style={inputStyle} required />
                            </div>
                    ...
                    <p style={{ fontSize: "0.75rem", color: "var(--color-text-muted)", textAlign: "center", marginTop: 16, lineHeight: 1.5 }}>
                        By signing up, you agree to our{" "}
                        <a href="/terms" style={{ color: "var(--color-primary-light)", textDecoration: "none" }}>Terms</a>{" "}and{" "}
                        <a href="/privacy" style={{ color: "var(--color-primary-light)", textDecoration: "none" }}>Privacy Policy</a>.
                    </p>
                </div>

                <p style={{ textAlign: "center", marginTop: 20, color: "var(--color-text-muted)", fontSize: "0.9rem" }}>
                    Already have an account?{" "}
                    <a href="/login" style={{ color: "var(--color-primary-light)", fontWeight: 600, textDecoration: "none" }}>Log in</a>
                </p>
            </div>
        </div>
    );
